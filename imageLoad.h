...
typedef struct
{
	...
	WORD CMapStart;		  // počátek barevné palety
	WORD CMapLength;	  // délka barevné palety
	BYTE CMapDepth;		  // bitová hloubka položek barevné palety
	WORD XOffset;		  // X-ová souřadnice počátku obrázku
	WORD YOffset;		  // Y-ová souřadnice počátku obrázku
	WORD Width;			  // šířka obrázku uvedená v pixelech
	WORD Height;		  // výška obrázku uvedená v pixelech
	BYTE PixelDepth;	  // počet bitů na jeden pixel (bitová hloubka)
	BYTE ImageDescriptor; //	popisovač obrázku
} TGAheader;
...
// funkce načítá jen bitmapové TGA, nekomprimované
